...
def calculate_junction_sr(junc, energies, bs, bsInitial, V, printParameters=False):
    ...
    jgen = q * bs_incident_on_depleted * (1 - exp(-alphaI * xi - alphaN * wn - alphaP * wp))  # jgen. Jenny, p. 159
    ...
    lifetime_n = ln ** 2 / dn  # these might not be the right lifetimes. Check ###
    lifetime_p = lp ** 2 / dp  # Jenny p163

    Jrec = q * ni * (wn + wp + xi) / sqrt(lifetime_n * lifetime_p) * sinh(q * V / (2 * kbT)) / (
    q * (Vbi - V) / kbT) * pi
